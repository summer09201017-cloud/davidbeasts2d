...
// 兩波野獸(撒上 17:34-36:先來了獅子,又來了熊)。同一套獅子引擎換皮+換數值。
//   lion 獅子=快靈;bear 熊=速度慢、傷害高(碰撞範圍大 contactBonus + 擊退大 knockbackMul)。
//   ★ 為兒童友善,兩波都只扣 1 顆心;熊的「傷害高」用「更難閃(碰撞範圍大)+ 擊退更遠」表達,
//     不用多扣心把小小孩勸退(見 beast-boss-kit §6 群獸公平鐵則的同款分寸)。
//   ★ 經文只有獅與熊——絕不加狼(撒上 17 沒有,見 beast-boss-kit §5)。
export const BEASTS = {
  lion: { label: '獅子', emoji: '🦁', speedMul: 1.0, contactBonus: 0, knockbackMul: 1.0 },
  bear: { label: '熊', emoji: '🐻', speedMul: 0.82, contactBonus: 10, knockbackMul: 1.45 },
}

// 野獸(boss)通用數值(兩波共用;熊在 BEASTS.bear 疊加差異)
export const LION = {
  r: 34, // 身體半徑
  maxHp: 30, // 有效反擊 30 下 → 觸發「制伏」收尾
  contactR: 56, // 衝撞命中大衛的距離(中心距);只有「衝刺(撲)」會造成傷害
...
// 救羊羔過場(撒上 17:35「我就追趕牠,擊打牠,將羊羔從牠口中救出來」)——
//   第一波獅子被制伏後、第二波熊登場前,大衛把羊羔從獸口中救回的短演出。
export const LAMB = { duration: 3.2 }
