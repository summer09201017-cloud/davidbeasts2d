import { ARENA, LION, BEASTS, PHASES, CORRUPTION, phaseOf } from './config.js'

// 野獸(boss):俯視角,在場上移動的單一物件狀態機。kind='lion'(第一波)或 'bear'(第二波)。
//   enter      登場:從場邊走向參孫
...
export class Lion {
  constructor() {
    this.kind = 'lion' // 'lion' | 'bear'(在 constructor 先設,reset 不覆寫 → 死亡回歸/復活後仍保持獸種)
    this.reset()
  }

  // 換獸種(第二波熊):只改種類,數值由 kindCfg 疊加。
  setKind(kind) {
    this.kind = BEASTS[kind] ? kind : 'lion'
    return this
  }

  kindCfg() {
    return BEASTS[this.kind] || BEASTS.lion
  }

  // 衝撞(撲)命中距離:熊體型大 → 碰撞範圍加大(更難閃 = 傷害感更重)
  contactR() {
    return LION.contactR + this.kindCfg().contactBonus
  }
  ...
  cfg() {
    ...
    const k = this.kindCfg().speedMul // 熊 <1 → 移動更慢;獅 =1
    // 時長縮短(÷e,狂暴/死神更兇)、移動加/減速(×e×獸種倍率);範圍與旗標不變。
    // 預警窗(telegraph/recovery)不受獸種影響 → 熊雖慢,但預警長度公平不變。
    return {
      telegraph: c.telegraph / e,
      recovery: c.recovery / e,
      approach: c.approach * e * k,
      charge: c.charge * e * k,
