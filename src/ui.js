...
export class UI {
  ...
  showTitle(L) {
    this.show(`
      <div class="kicker">大衛打獅與熊 · 撒母耳記上 17</div>
      <h1>${L.title}</h1>
      <p class="sub">${L.subtitle}</p>
      <div class="verse"><span class="ref">${L.ref}</span>${L.verse}</div>
      <div class="row">
        <button class="btn" data-act="start">🐑 開始保護羊群</button>
      </div>
      <p class="hint">
        🏃 <b>走位</b>:WASD / 方向鍵,或<b>在畫面上拖曳</b> —— 上下左右自由移動,側身閃開野獸的撲擊。<br>
        👊 <b>反擊</b>:空白 / Enter / J,或<b>輕點畫面</b> —— 靠近野獸、趁牠撲完喘氣(💢)時出手。<br>
        兩波:先制伏<b>獅子</b> → 救回羊羔 → 再趕走<b>熊</b>。看準「預備(⚠)→ 撲 → 喘氣(💢)」的節奏,閃開再靠近反擊!
      </p>
    `)
  }
  ...
  showWin(L, info = {}) {
    ...
    const wv = (L.win && L.win.verse) || L.verse
    const wr = (L.win && L.win.ref) || L.ref
    this.show(`
      <div class="kicker win">得勝了!</div>
      <h2>${L.win.head}</h2>
      <p class="body">${L.win.body.replace(/\n/g, '<br>')}</p>
      <div class="verse"><span class="ref">${wr}</span>${wv}</div>
      ${hearts ? `<p class="score">剩餘體力 ${hearts}</p>` : ''}
      <div class="row">
        <button class="btn ghost" data-act="restart">↻ 再玩一次</button>
        <button class="btn" data-act="honey">📖 牧人的詩</button>
      </div>
    `)
  }
