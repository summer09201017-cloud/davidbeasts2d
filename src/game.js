import { SAMSON, LION, HONEY, ROCK, MIRACLE, CORRUPTION, GOLDEN_HEART, ESCAPE, ARENA, INTRO, FINISHER, LAMB, BADEND } from './config.js'
...
const STATE = {
  ...
  LAMB: 'lamb', // 救羊羔過場:第一波獅子被制伏 → 大衛救回羊羔 → 第二波熊登場(不可失敗)
  FINISHER: 'finisher', // 「制伏」收尾(不可失敗,玩家不操控)
...
export class Game {
  ...
  constructor(canvas, opts = {}) {
    ...
    this.samson = new Samson() // 玩家(少年大衛;內部沿用底座類名)
    this.lion = new Lion() // 野獸(第一波獅子 / 第二波熊;內部沿用底座類名)
    this.wave = 1 // 1 = 獅子波、2 = 熊波(撒上 17:34-36:先獅子、後熊)
    this.state = STATE.TITLE
    this.combo = 0 // 連續有效反擊(視覺爽感用)
    this.intro = { t: 0 }
    this.lamb = { t: 0 } // 救羊羔過場計時
    this.fin = { t: 0 }
  ...
  // 開始一場全新戰鬥(從第一波獅子開始);重試 / LOSE 續玩都走這裡。
  startFight() {
    this._enterImmersive()
    this.wave = 1
    this.samson.reset()
    this.lion.reset()
    this.lion.setKind('lion')
    this._resetBattleField()
    this.lion.deathMode = this.deathMode // 死神模式:野獸全面強化(見 lion.cfg / renderer)
    this.state = STATE.FIGHT
    window.psPing?.('davidbeasts2d-start'); this._psT0 = Date.now()
  ...
  // 第二波:熊登場(接續同一場戰鬥;不清 deaths/deathMode,救了羊羔後補滿體力重整旗鼓)
  _startBearWave() {
    this.wave = 2
    const keepMax = this.samson.maxHearts // 金色的心突破的上限是永久增益,跨波保留
    this.samson.reset()
    this.samson.maxHearts = keepMax
    this.samson.hearts = keepMax
    this.lion.reset()
    this.lion.setKind('bear')
    this._resetBattleField()
    this.lion.deathMode = this.deathMode
    this.state = STATE.FIGHT
    this.hudLabels = this._hudOverride || {
      ...LEVEL1.hud,
      ...(this.deathMode && LEVEL1.deathHud ? LEVEL1.deathHud : {}),
  ...
  // 重置戰場的特效/計時/拾取物(兩波共用;不動 samson/lion 本體與 deaths/deathMode)
  _resetBattleField() {
    this.combo = 0
  ...
  // 偵錯:直接跳到指定波次(僅 localhost 由 window.__game 呼叫,正式流程不會用到)
  startWaveDebug(n) {
    if (n === 2) {
      this.deaths = 0
  ...
  step(dt) {
    ...
    // A) 野獸衝撞(撲)大衛(只有 charge 會造成傷害;走近不會)。熊碰撞範圍大、擊退更遠。
    if (l.charging() && s.invuln <= 0 && d < l.contactR()) {
      s.hurt(l.x, l.y, l.kindCfg().knockbackMul)
      this.fx.hurtT = 0.4
      Audio.sfx('hit')
      if (s.hearts <= 0) {
        this.gameOver()
  ...
  // 制伏收尾演完:第一波(獅子)→ 救羊羔過場 → 熊登場;第二波(熊)→ 得勝
  _afterFinisher() {
    if (this.wave >= 2) return this.win()
    if (this.embed) return this._startBearWave() // 嵌入無過場演出,直接接第二波
    this.enterLamb()
  }

  // 救羊羔過場(撒上 17:35):大衛把羊羔從獸口中救出來,再迎向逼近的熊
  enterLamb() {
    this.state = STATE.LAMB
    this.lamb = { t: 0 }
    this.ui.hidePauseButton()
    Audio.stopMusic()
    Audio.sfx('heal') // 救回羊羔的溫柔提示音
  }
  ...
  loop(t) {
    ...
    if (this.state === STATE.FIGHT) {
      ...
      if (this.fin.t >= FINISHER.duration) this._afterFinisher()
    } else if (this.state === STATE.LAMB) {
      // 救羊羔過場(撒上 17:35):可輕點跳過,時間到 → 熊登場
      this.lamb.t += dt
      const skip = this.input.consumeSkip()
      this.input.consumeAttack()
      if (skip || this.lamb.t >= LAMB.duration) this._startBearWave()
  ...
  win() {
    // 結尾朗讀 17:37「耶和華救我脫離獅子和熊的爪」(得勝歸神)
    speakScripture(LEVEL1.win?.verse || LEVEL1.verse, { ref: LEVEL1.win?.ref || LEVEL1.ref })
    this.state = STATE.WIN
    window.psPing?.('davidbeasts2d-done', this._psT0 ? Math.round((Date.now() - this._psT0) / 1000) : 0)
    this.ui.hidePauseButton()
    Audio.stopMusic()
    Audio.sfx('win')
