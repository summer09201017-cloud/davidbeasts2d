import { VIEW, ARENA, SAMSON, LION, BEASTS, HONEY, ROCK, CORRUPTION, GOLDEN_HEART, ESCAPE, INTRO, FINISHER, LAMB, BADEND } from './config.js'
...
export class Renderer {
  ...
  _drawArena(game, t) {
    ...
    const drawLion = () => this._lion(l.x, l.y, l.face, l.state, l.phase(), t, l.flash, l.open, l.deathMode, l.kind)
    if (s.y <= l.y) {
      drawSamson()
      drawLion()
    } else {
  ...
  _drawIntro(game, t) {
    ...
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    ctx.fillText('一隻獅子撲向大衛的羊群!　（輕點畫面跳過)', VIEW.W / 2, VIEW.H - 39)
  }
  ...
  _drawFinisher(game, t) {
    ...
    this._lion(l.x, l.y, l.face, 'recovery', l.phase(), t, 0, false, l.deathMode, l.kind)
    ctx.restore()
    ctx.globalAlpha = 1
    ...
    if (vk > 0) {
      ...
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
      ctx.fillText('耶和華救大衛脫離獸的爪', VIEW.W / 2, 58)
      ctx.globalAlpha = 1
    }
  }

  // ---- 救羊羔過場(撒上 17:35):大衛把羊羔從獸口中救出;右側熊的剪影正逼近 ----
  _drawLamb(game, t) {
    const ctx = this.ctx
    const p = Math.min(1, game.lamb.t / LAMB.duration)
    this._bgArena(t)
    // 柔和暖光罩
    ctx.fillStyle = 'rgba(20,12,4,0.26)'
    ctx.fillRect(0, 0, VIEW.W, VIEW.H)

    const sx = ARENA.x + ARENA.w * 0.4
    const sy = ARENA.y + ARENA.h * 0.66
    // 熊的剪影從右側逼近(預示第二波),隨 p 走近
    const bx = VIEW.W - 30 - p * 130
    const by = ARENA.y + ARENA.h * 0.52
    ctx.globalAlpha = 0.35 + 0.4 * p
    this._lion(bx, by, -1, 'approach', 0, t, 0, false, false, 'bear')
    ctx.globalAlpha = 1

    // 大衛(站立,面向右),懷中抱著救回的羊羔
    this._samson(sx, sy, 1, 'idle', 0, false, 0)
    this._lamb(sx + 17, sy - 38, 1)

    // 字幕
    const L = LEVEL1.lamb || {}
    ctx.fillStyle = 'rgba(40,20,8,0.62)'
    roundRect(ctx, VIEW.W / 2 - 360, VIEW.H - 84, 720, 62, 14)
    ctx.fill()
    ctx.fillStyle = '#ffe9b0'
    ctx.font = `800 22px ${FONT}`
    ctx.textAlign = 'center'
    ctx.textBaseline = 'middle'
    ctx.fillText(`${L.head || '將羊羔從牠口中救出來'}　（${L.ref || '撒母耳記上 17:35'}）`, VIEW.W / 2, VIEW.H - 63)
    ctx.fillStyle = '#ffe9cf'
    ctx.font = `600 16px ${FONT}`
    ctx.fillText(L.caption || '', VIEW.W / 2, VIEW.H - 38)

    ctx.fillStyle = 'rgba(255,233,207,0.7)'
    ctx.font = `600 13px ${FONT}`
    ctx.textBaseline = 'bottom'
    ctx.fillText('（輕點畫面跳過)', VIEW.W / 2, VIEW.H - 8)
  }

  // 小羊羔(被大衛抱在懷中):蓬鬆白毛 + 小頭 + 臉
  _lamb(x, y, face) {
    const ctx = this.ctx
    ctx.save()
    ctx.translate(x, y)
    ...
    for (const a of [-0.5, 0.3, 1.0, 1.8, 2.6, 3.3, 4.0]) {
      ctx.beginPath()
      ctx.arc(Math.cos(a) * 12, Math.sin(a) * 9, 4.5, 0, Math.PI * 2)
      ctx.fill()
    }
    // 身體
    ...
    ctx.beginPath()
    ctx.ellipse(-11, -6, 5.6, 4.8, -0.3, 0, Math.PI * 2)
    ctx.fill()
    // 耳
    ctx.fillStyle = '#d3c7b0'
    ctx.beginPath()
    ctx.ellipse(-8, -9, 2.6, 1.6, 0.6, 0, Math.PI * 2)
    ctx.fill()
    // 臉:眼(有臉)
  ...
  // ---- 少年大衛(牧羊人;俯視站立小人,牧杖+投石帶+短袍,有臉:眼耳嘴眉)----
  _samson(x, footY, face, action, actionT, moving, walkPhase) {
    const ctx = this.ctx
    const SKIN = '#eab488' // 少年大衛(撒上 16:12 面色光紅)
    const SKIN_D = '#cf9159'
    const HAIR = '#7a4a24' // 紅褐短髮
    const HAIR_HI = '#9c6631'
    const TUNIC = '#d9c49a' // 牧羊人淺褐短袍
    const TUNIC_D = '#b39a6c'
    const TUNIC_HI = '#ece0c0'
    const BELT = '#7a5228'
    const STAFF = '#6e4a24' // 牧杖
    const STAFF_HI = '#8f6330'
    const POUCH = '#7c5330' // 投石帶的皮囊
    ...
    ctx.beginPath()
    ctx.moveTo(-17, -1)
    ctx.lineTo(-21, headY - 20)
    ctx.stroke()
    // 曲柄(往前彎的鉤)
    ctx.beginPath()
    ctx.moveTo(-21, headY - 20)
    ctx.quadraticCurveTo(-21, headY - 30, -13, headY - 28)
    ctx.quadraticCurveTo(-8, headY - 27, -9, headY - 21)
    ctx.stroke()
    ctx.strokeStyle = STAFF_HI
    ctx.lineWidth = 1.2
    ctx.beginPath()
    ctx.moveTo(-18.6, -1)
    ctx.lineTo(-22.4, headY - 19)
    ctx.stroke()
    ...
    // ---- 投石帶:腰間垂下的皮囊 + 兩條繩帶(大衛的兵器)----
    ctx.strokeStyle = '#5e3e1e'
    ctx.lineWidth = 1.5
    ctx.lineCap = 'round'
    ctx.beginPath()
    ctx.moveTo(-6, hipY)
    ctx.lineTo(-8, hipY + 11)
    ctx.moveTo(-2, hipY)
    ctx.lineTo(-8, hipY + 11)
    ctx.stroke()
    ctx.fillStyle = POUCH
    ctx.beginPath()
    ctx.ellipse(-8, hipY + 13, 4.2, 5, 0, 0, Math.PI * 2)
    ctx.fill()
    ctx.fillStyle = 'rgba(0,0,0,0.18)'
    ctx.beginPath()
    ctx.ellipse(-8, hipY + 14.5, 3, 2.4, 0, 0, Math.PI * 2)
    ctx.fill()
    ...
    // ---- 短髮(少年牧童的紅褐短捲髮,不留長髮、無頭帶)----
    ctx.fillStyle = HAIR
    // 頭頂髮罩(蓋住上半頭,前緣露出額頭)
    ctx.beginPath()
    ctx.arc(0, headY - 1, headR + 2, Math.PI * 0.86, Math.PI * 2.12)
    ctx.fill()
    // 鬢角(耳前一小撮,短)
    ctx.beginPath()
    ctx.moveTo(-headR + 1, headY - 3)
    ctx.quadraticCurveTo(-headR - 2, headY + 2, -headR + 1, headY + 5)
    ctx.quadraticCurveTo(-2, headY + 2, -2, headY - 2)
    ctx.closePath()
    ctx.fill()
    // 短髮捲(頭頂幾撮高光)
    ctx.fillStyle = HAIR_HI
    for (const hx of [-4, 0, 4]) {
      ctx.beginPath()
      ctx.arc(hx, headY - headR + 1, 2.1, 0, Math.PI * 2)
      ctx.fill()
    }

    // ---- 臉(朝右):眉、眼、耳、鼻、嘴(少年,無鬍;l6 鐵則:人物一定要有臉)----
    // 耳朵(3/4 側臉露一隻耳)
    ctx.fillStyle = SKIN
    ctx.beginPath()
    ctx.arc(-2, headY + 1, 2.6, 0, Math.PI * 2)
    ctx.fill()
    ctx.strokeStyle = SKIN_D
    ctx.lineWidth = 0.8
    ctx.beginPath()
    ctx.arc(-2, headY + 1, 1.3, -1.2, 1.6)
    ctx.stroke()
    // 眉
    ...
    ctx.lineCap = 'round'
    ctx.beginPath()
    ctx.moveTo(headR - 5.5, headY - 3.6)
    ctx.lineTo(headR - 0.5, headY - 3)
    ctx.stroke()
    // 眼
    ctx.fillStyle = '#2a1a0e'
    ctx.beginPath()
    ctx.arc(headR - 2.6, headY - 1, 1.5, 0, Math.PI * 2)
    ctx.fill()
    // 鼻
    ctx.strokeStyle = SKIN_D
    ctx.lineWidth = 1.6
    ctx.beginPath()
    ctx.moveTo(headR - 0.3, headY)
    ctx.lineTo(headR + 1.6, headY + 2)
    ctx.lineTo(headR - 1, headY + 2.6)
    ctx.stroke()
    // 嘴(小小的微笑線)
    ctx.strokeStyle = '#9c5238'
    ctx.lineWidth = 1.3
    ctx.beginPath()
    ctx.moveTo(headR - 4, headY + 4.6)
    ctx.quadraticCurveTo(headR - 2, headY + 6, headR + 0.2, headY + 4.8)
    ctx.stroke()
  ...
  _lion(x, footY, face, state, phase, t, flash, open, deathMode = false, kind = 'lion') {
    ...
    const walk = walking ? Math.sin(t * 9) : 0
    let bodyY = -46
    let bodyW = isBear ? 60 : 64 // 熊:略短
    let bodyH = isBear ? 37 : 30 // 熊:更厚更壯
    let headX = -54
    let headY = -52
    let rear = 0
    if (state === 'telegraph') {
    ...
    // 尾巴:獅=長尾+末端毛球;熊=短尾(小凸)
    if (isBear) {
      ctx.fillStyle = BODY_D
      ctx.beginPath()
      ctx.arc(bodyW * 0.82, bodyY + 8, 5.5, 0, Math.PI * 2)
      ctx.fill()
    } else {
    ...
    if (isBear) {
      ctx.fillStyle = BODY_HI
      ctx.beginPath()
      ctx.ellipse(-20, bodyY - bodyH * 0.5, 22, 15, -0.18, 0, Math.PI * 2)
      ctx.fill()
    }
    ...
    // 鬃毛(只有獅子有;熊去鬃):外圈長毛(深)+ 內圈短毛(淺),雙層更厚實
    if (!isBear) {
      const mx = headX + 12
      const my = headY + 6
      const spikes = 15
    ...
    if (isBear) {
      // 熊:大而圓的耳朵,高踞頭頂兩側(招牌圓耳)
      drawEar(headX + 12, headY - 15, 8) // 後耳(遠)
      drawEar(headX - 8, headY - 16, 8.5) // 前耳(近)
    } else {
  ...
  _hud(game, t) {
    ...
    // 獸名/圖示由當前獸種決定(獅子🦁 / 熊🐻);死神模式一律顯示死蔭💀
    const beast = BEASTS[l.kind] || BEASTS.lion
    const goal = l.deathMode ? (game.hudLabels && game.hudLabels.goal) || '死蔭' : beast.label
    const beastEmoji = l.deathMode ? '💀' : beast.emoji
    ctx.fillStyle = l.deathMode ? '#e3b6ff' : '#f3ead0'
    ctx.font = `700 15px ${FONT}`
    ctx.textAlign = 'center'
    ctx.textBaseline = 'bottom'
    ctx.fillText(`${beastEmoji} ${goal}　·　第 ${game.wave || 1} / 2 波`, VIEW.W / 2, by - 6)
    ...
    ctx.textBaseline = 'bottom'
    ctx.fillText(
      'WASD／方向鍵(或拖曳畫面)走位閃避　·　靠近野獸、趁牠喘氣 💢 時攻擊(空白／點一下)',
      VIEW.W / 2,
      VIEW.H - 10
    )
  }
